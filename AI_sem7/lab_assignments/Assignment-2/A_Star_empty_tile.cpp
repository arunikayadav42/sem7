...
set<vector<vector<int>>> closedList1, closedList2, closedList3, closedList4;
...
void aStarSearchHeuristicOne(Node *start, Node *goal){
    for(int heuristic = 1; heuristic <=4 ; heuristic++){
        ...
        if(found == -1){
            ...
            cout<<"Total Number of nodes explored before termination : "<<closedList.size()<<"\n";
            stack<Node*> optimalPath =  printOptimalPath(goal);
...
int main(){
    ...
    cout <<"Expansion : h1 > h2 : " << includes(closedList1.begin(), closedList1.end(), closedList2.begin(), closedList2.end()) << endl;
    cout <<"Expansion : h2 > h3 : " << includes(closedList2.begin(), closedList2.end(), closedList3.begin(), closedList3.end()) << endl;
